...
void PruningStates::load() {
    ...
    if (this->path != "" && boost::filesystem::exists(path)) {
        ...
        if (cfg->useMmapForPruning) {
            ...
            cfg->log <<  "Creating pruning table (" << path << ") of size " << estimateSizeInGb() << " Gb in memory...\n\n";
            data = new uint8_t[siz];
        }
...
string PruningStates::getStats(
) { // TODO: store stats so we dont have to look through the whole TABLE (potentially several Gib)
    uint64_t siz = ((uint64_t)1) << hashSize;
    stringstream ss;
    ...
    if (needsStats) {
        for (uint64_t i = 0; i < siz; i++) { stats[data[i]]++; }
